...
async function fetchDiff(repo: string, prNumber: number): Promise<string | null> {
  ...
  return text.slice(0, 5000).trim() || null;
}
...
export async function getOutcome(repo: string, prNumber: number): Promise<string> {
  const res = await fetch(
    `https://api.github.com/repos/${repo}/pulls/${prNumber}`,
    { headers: headers() },
  );
